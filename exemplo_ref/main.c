...
//assume que há pelo menos 2 nós
void func2(node **p){
    node *seg = (*p)->next;
    node *third = &(seg->next);
...
int main() {
    ...
    secondNode->next = (node *) malloc(sizeof(node));;    //p->next->next = NULL;


    node *thirdNode = secondNode->next;
    thirdNode->key = 12;       //p->next->key = 11;
    thirdNode->data = 'm';     //p->next->data = 'n';
    thirdNode->next = NULL;    //p->next->next = NULL;
    printf("%d\n", &(thirdNode->next));
    func2(&p);
